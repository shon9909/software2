...
public class ServiceRegistro{
	...
	//Aquí se registran los datos ingresados de la medicacion
	@POST
	...
	@POST
	@Path("/borrarMedicacion")
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public Response borraMedicacion(VOmedicacion medicacion) throws SQLException, JSONException{

		DAOmedicacion daomedicacion=new DAOmedicacion();
		String a=daomedicacion.borrarMedicacion(medicacion.getId_medicacion());
		if(a.isEmpty()!=true){
			return Response.status(Response.Status.CREATED).entity("{\"Status\": \"hecho\"}").build();
		}else{
			return Response.status(Response.Status.CREATED).entity("{\"Status\": \"error al borrar\"}").build();
		}
